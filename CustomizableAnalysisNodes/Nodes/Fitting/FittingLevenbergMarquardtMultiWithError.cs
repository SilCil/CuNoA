...
{
    [Node("フィッティング/任意関数の和/レーベンバーグ・マーカート法（推定値の標準偏差含む）")]
    public class FittingLevenbergMarquardtMultiWithError : ICalculationNode, IOptionNode
    {
...
";
        ...
        public double[] ResultingParameterErrors { get; set; } = { 1.0 };
        ...
        private string GenerateKey(string code) => $"{nameof(CustomizableAnalysisLibrary.Nodes)}{nameof(FittingLevenbergMarquardtMultiWithError)}:{code}";
        ...
        public Table Run(Table data)
        {
            ...
            ResultingParameterErrors = result.StandardErrors.ToArray();
            ...
            if (OutputCurve)
            {
                ...
                columns.Add(result.MinimizedValues.ToValueArray());
            ...
            {
                return Table.CreateFromColumns(ResultingParameters.ToValueArray(), ResultingParameterErrors.ToValueArray());
            }
        }
    }
}
